...
#' @description
#' Funkcja drukuje kod latex dla szablonu tabeli z rysunkami zadanymi przez
#' wektor ścieżek do plików.
...
#' @param szerokosc szerokość tabeli z rysunkami. Jeden oznacza całą szerokość
#' strony.
#' @return
...
drukuj_rysunki = function(plikiRys, wymiar, znacznikiTab, tabelaSzablonTex,
  ...
  for (k in 1:wymiar[1]) {
    for (n in 1:wymiar[2]) {
      ...
      tab[k,n] =  paste0("\\includegraphics[width=\\textwidth*\\real{",
                         szerokosc/wymiar[1],"}]{",
                         plikiRys[(k - 1)*wymiar[2] + n], "}")
    }
  }
  drukuj_tabele(tab, tabelaSzablonTex, znacznikiTab, naglowek = FALSE,
                zamien_znaki = FALSE)
  invisible(NULL)
}
