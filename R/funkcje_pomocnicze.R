...
#' @description
#' Drukuje szablon z pliku oraz zamienia znaczniki na ciągi znakowe zdefiniowane
#' przez użytkownika. Jeśli w ramach pakietu został zdefiniowany plik
#' z szablonem o nazwie takiej, jak podana argumentem \code{plikSzablonuTex},
#' zostanie on wczytany z zasobów pakietu. W przeciwnym wypadku, argument
#' \code{plikSzablonuTex} zostanie potraktowany jako ścieżka do pliku
#' z szablonem względem aktywnego katalogu roboczego.
...
drukuj_szablon = function(plikSzablonuTex, znaczniki){
  ...
  temp = paste0(temp[temp[, 1] == "EWDraport", 2], "/EWDraport/", plikSzablonuTex)
  if (file.exists(temp)) {
    con = file(temp, open = "r")
  } else if (file.exists(plikSzablonuTex)) {
    con = file(plikSzablonuTex, open = "r")
  } else {
    stop("Nie daje się otworzyć pliku '", plikSzablonuTex, "'. ",
...
drukuj_wiersz_tabeli = function(linia, wektor){
  ...
  cat(spRes[[1]][1], " ", paste0(paste(wektor, collapse = " & "), " ",
                                 spRes[[1]][2]), "\n")
  invisible(NULL)
}
#' @title Drukuj wiersz
...
zamien_znaczniki = function(nazwa, znaczniki){
  ...
  for (i in seq_along(znaczniki)) {
    if (grepl("Plik", names(znaczniki)[i])) {
      ...
      ret = gsub(paste0("!", names(znaczniki)[i]),
                 zamien_znaki_tex(znaczniki[[i]]), ret, fixed  = TRUE)
    }
  }
  return(ret)
}
...
#' @description
#' Funkcja przekształca ciąg znaków zastępując wybrane znak specjalne na kod
#' tex, który pozwala na wyświetlenie ich w dokumencie tex.
...
#' @title Zamien znaki
#' @description
#' Funkcja przekształca ciąg znaków zastępując podciągi (zadane argumentem
#' \code{strOrg}) na odpowiadające im podciągi zadane argumentem
#' \code{znakiNowe}.
#' @param nazwa ciąg znaków do zmiany
#' @param strOrg wektor ciągów znaków do zastąpienia.
#' @param strN wektor ciągów znaków zastępujące odpowiednie elementy z
#' \code{strOrg}.
#' @return Przekształcony ciąg znaków.
#' @export
...
#' @param envir środowisko, gdzie znajduje się zmienna o nazwie
#' nazwaZmiennejGlobalnej
#' @return
#' Funkcja zwraca obiekt klasy 'NumeracjaTex'.
#' @export
inicjalizuj_numeracje = function(nazwaZmiennejGlobalnej = "numeracjaTexZG",
                                 envir = .GlobalEnv){
...
#' @describeIn inicjalizuj_numeracje alias (pierwotna nazwa nadana przez Grześka)
initializuj_numeracje = function(nazwaZmiennejGlobalnej = "numeracjaTexZG",
                                 envir = .GlobalEnv){
  inicjalizuj_numeracje(nazwaZmiennejGlobalnej, envir)
}
#' @title Numeracja
#' @description
#' Funkcja dokleja do parametru nazwa numer, który oznacza kolejność wywołania
#' funcji z tym parametrem.
...
numeracja = function(nazwa, tylkoNumer = FALSE,
                     ...
                     envir = .GlobalEnv){
  if (!nazwaZmiennejGlobalnej %in% ls(envir = envir) ||
      class(get(nazwaZmiennejGlobalnej)) != "NumeracjaTex") {
  ...
  if (!nazwa %in% numeracjaTex$nazwa) {
    numeracjaTex = rbind(numeracjaTex, data.frame(nazwa = nazwa, numer = 0,
