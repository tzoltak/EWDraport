...
ustaw_naglowek_tabeli("Liczba zdających poszczególne części egzaminu.")
pander_table(tmpTab)

if (egz == "g") {
  ustaw_naglowek_tabeli("Liczba zdających części humanistyczne w podziale na lata pisania egzaminu gimnazjalnego i sprawdzianu.")
  tab = tabelki$liczbaZdajacychPoLatach
  tab = tab[, grepl("^rok_g$|^rok_s$|^zdający$|^gh", colnames(tab))]
  pander_table(tab)
  
  ustaw_naglowek_tabeli("Liczba zdających części matematyczne w podziale na lata pisania egzaminu gimnazjalnego i sprawdzianu.")
  tab = tabelki$liczbaZdajacychPoLatach
  tab = tab[, grepl("^rok_g$|^rok_s$|^zdający$|^gm", colnames(tab))]
  pander_table(tab)
} else if (egz == "m") {
  ustaw_naglowek_tabeli("Liczba zdających egzamin w podziale na lata pisania egzaminu gimnazjalnego i matury.")
  tab = tabelki$liczbaZdajacychPoLatach
  pander_table(tab)
}
```
...
for (ind in which(grepl("^wy|^we", names(tabelki)))) {
  ...
  ustaw_naglowek_tabeli(naglowek)
...
for (ind in seq_along(modele)) {
  ...
  if ("linkTest" %in% names(porownania)) {
    ustaw_naglowek_tabeli(paste0("Linktest modeli ", modelu[nazwaModelu], "."))
    pander_table(porownania$linkTest)
  }
  if ("czyRosnaca" %in% names(porownania)) {
    ustaw_naglowek_tabeli(paste0("Monotoniczność funkcji przewidywania modeli ",
                               modelu[nazwaModelu], "."))
    pander_table(porownania$czyRosnaca)
  }
  if ("statystyki" %in% names(porownania)) {
    ustaw_naglowek_tabeli(paste0("Statystyki dopasowania modeli ",
                               modelu[nazwaModelu], "."))
  ...
  for (st in stopnie) {
    cat(paste0("\n\\subsection{Wielomian ", st, "-ego stopnia dla modelu ",
               modelu[nazwaModelu], "}\\label{StopienWielomianu-,",
               nazwaModelu, "}\n"))
  ...
  ustaw_naglowek_tabeli("Statystyki dopasowania modelu EWD.")
  pander(schowek$AICtab)

  ustaw_naglowek_tabeli("Dekompozycja wariancji modelu EWD.")
  schowek$efLos[, 2] =
  ...
  ustaw_naglowek_tabeli(paste0("Parametry efektów stałych modelu ",
                             modelu[nazwaModelu], "."))
  ...
  colnames(schowek$efSt) = c("Zmienna", "Wartość", "Odch. Std.", "Statystyka t")
